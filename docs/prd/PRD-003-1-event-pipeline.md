...
| `on_tool_start` | `metadata["langgraph_node"] in WORKER_NODES` AND `event["name"] == "tavily_search"` AND `langgraph_node in spawned_agents` | `agent.tool_call` | Same payload as above; tool_name will be `"tavily_search"` | **Requires `ToolNode`** — not emitted by `RunnableLambda`; see PRD-004-1 §5 |
| `on_tool_end` | `metadata["langgraph_node"] in WORKER_NODES` AND `event["name"] == "tavily_search"` AND `langgraph_node in spawned_agents` | `agent.tool_result` | `agent_id`: from `spawned_agents`; `tool_name`: `"tavily_search"`; `result_summary`: `str(data["output"])[:120]` | **Requires `ToolNode`** — not emitted by `RunnableLambda` |
