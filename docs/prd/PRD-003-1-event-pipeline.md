...
| `on_tool_start` | `event["name"] == "tavily_search"` | `agent.tool_call` | Same payload as above; tool_name will be `"tavily_search"` | **Requires `ToolNode`** — not emitted by `RunnableLambda`; see PRD-004-1 §5 |
| `on_tool_end` | `event["name"] == "tavily_search"` | `agent.tool_result` | `data["output"]` is the raw Tavily result list | **Requires `ToolNode`** — not emitted by `RunnableLambda` |
